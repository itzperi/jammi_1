            ...
            <tbody className="divide-y divide-slate-100">
              {[
                ...
                <tr key={p.sku} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className={`px-3 py-1 inline-flex rounded-full text-xs font-bold items-center gap-1.5 ${p.sColor === 'green' ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${p.sColor === 'green' ? 'bg-amber-500' : 'bg-red-500'}`}></span> {p.status}
                    </div>
